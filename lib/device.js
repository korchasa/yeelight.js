...
var Device = function () {
  ...
  function Device(payload) {
    ...
    this.logger = new _logger2.default({ enabled: payload.verbose || false });
  }
  ...
  _createClass(Device, [{
    ...
    value: function sendCommand(command) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        if (undefined === command.id) {
          newCommand.id = _this.counter + 1;
          _this.commands.push(newCommand);
        }

        var stringified = JSON.stringify(newCommand);
        ...
        socket.on('data', function (data) {
          ...
          lines.forEach(function (line) {
            var response = _this.unserialize(line);
            if (!response) {
            ...
            if (response.id === newCommand.id) {
              _this.commands = _this.commands.filter(function (comm) {
    ...
    key: 'setCtAbx',
    value: function setCtAbx(ctValue) {
    ...
    key: 'setRgb',
    value: function setRgb(rgbValue) {
      var effect = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'smooth';
      var duration = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 500;
